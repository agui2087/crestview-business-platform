...
import {parseSignatureAppearance} from '@/lib/nda-signature';
...
export default async function Sign({params}:{params:Promise<{locale:string;id:string}>}) {
 ...
 const state=signingState(nda.status,controls),ready=['sent','viewed'].includes(state),es=locale==='es';
 const {data:signatures,error:signatureError}=await db.from('deal_nda_signatures').select('role,legal_name,signed_at,field_values,appearance').eq('nda_id',nda.id);if(signatureError)throw Error('Signature progress unavailable');
