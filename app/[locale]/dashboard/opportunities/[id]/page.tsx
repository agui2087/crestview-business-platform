      ...
      <div className="dashboard-content acquisition-detail-page">
        ...
        {workspace && workspace.stage !== "saved" && <div className="deal-operations">
          <section className="operations-card" id="diligence">
            ...
            <p>{es?'Los estados los registras tú; no representan verificación independiente de Crestview. Las descargas incluyen tus notas privadas. Revísalas y confirma que tienes permiso antes de compartirlas.':'Statuses are recorded by you, not independently verified by Crestview. Downloads include your private notes. Review them and confirm you have permission before sharing.'}</p>
            <div className="detail-actions"><a className="button button--light" href={`/api/export/diligence?opportunity=${encodeURIComponent(opportunity.id)}&locale=${locale}`}>{es?'Descargar mi diligencia (CSV)':'Download my diligence (CSV)'}</a><a className="button button--light" href={`/api/export/diligence?opportunity=${encodeURIComponent(opportunity.id)}&locale=${locale}&unresolved=1`}>{es?'Descargar elementos sin resolver (CSV)':'Download unresolved items (CSV)'}</a></div>
