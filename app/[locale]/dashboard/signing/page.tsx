...
export default async function SigningCenter({params,searchParams}:{params:Promise<{locale:string}>;searchParams:Promise<{state?:string;q?:string;page?:string}>}) {
 ...
 const page=/^[1-9]\d{0,5}$/.test(query.page??'')?Number(query.page):1,search=(query.q??'').slice(0,160);
 let lookup=supabase.from('deal_ndas').select('id,inquiry_id,document_name,template_version,status,buyer_id,sent_at,signed_at,signer_name',{count:'exact'}).or(`buyer_id.eq.${user.id},broker_id.eq.${user.id}`).order('created_at',{ascending:false}).order('id',{ascending:false});
 if(search)lookup=lookup.ilike('document_name',`%${search.replace(/[\\%_]/g,character=>'\\'+character)}%`);
 const {data:ndas,error,count}=await lookup.range((page-1)*100,page*100-1);
 const pageLink=(next:number)=>`/${locale}/dashboard/signing?${new URLSearchParams({page:String(next),q:search,state:query.state??'all'})}`;
 ...
 return <PlatformShell locale={locale} active="signing"><div className="dashboard-content">
  ...
  <p>{t('Showing up to 100 agreements per page. Name search includes older agreements; status filters apply to this page. Reminders are in-app, not email. Prepared PDFs can require both buyer and broker signatures.','Se muestran hasta 100 acuerdos por página. La búsqueda incluye acuerdos anteriores; el filtro de estado se aplica a esta página. Los recordatorios son internos, no por correo. Los PDF preparados pueden requerir las firmas del comprador y del corredor.')}</p>
  ...
  <nav aria-label={t('Agreement history pages','Páginas del historial')}>{page>1&&<Link href={pageLink(page-1)}>{t('Previous page','Página anterior')}</Link>}<span> {t('Page','Página')} {page} </span>{page*100<(count??0)&&<Link href={pageLink(page+1)}>{t('Next page','Página siguiente')}</Link>}</nav>
