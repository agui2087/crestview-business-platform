...
export default async function SigningCenter({params,searchParams}:{params:Promise<{locale:string}>;searchParams:Promise<{state?:string;q?:string}>}) {
 ...
 return <PlatformShell locale={locale} active="signing"><div className="dashboard-content">
  ...
  <p>{t('Showing your latest 100 agreements. Reminders are in-app, not email. Prepared PDFs can require both buyer and broker signatures.','Se muestran tus 100 acuerdos más recientes. Los recordatorios son internos, no por correo. Los PDF preparados pueden requerir las firmas del comprador y del corredor.')}</p>
