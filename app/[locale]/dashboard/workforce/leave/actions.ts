...
export async function leaveOperation(form:FormData) {
  ...
  if(get('operation')==='transfer'&&get('confirmed')==='yes'&&/^\d+$/.test(get('minutes'))&&/^-?\d+$/.test(get('expected_balance'))) {
    const [target,version]=get('target').split(':');
    const r=await db.rpc('workforce_transfer_leave',{p_source:policy,p_target:target,p_source_version:Number(get('version')),p_target_version:Number(version),p_expected_balance:Number(get('expected_balance')),p_minutes:Number(get('minutes')),p_reason:get('reason')});failed=Boolean(r.error);
  }
