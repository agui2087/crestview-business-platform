...
export default async function VisionPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const journey = es ? ["Encontrar", "Evaluar", "Adquirir", "Operar"] : ["Find", "Evaluate", "Acquire", "Operate"];
  return <><MarketingHeader locale={locale} /><main>
    <section className="marketing-page-hero vision-hero">
      <div className="shell vision-hero__grid">
        <div className="vision-hero__copy">
          <p className="eyebrow">{es ? "Nuestra visión" : "Our vision"}</p>
          <h1>{es ? "La propiedad de negocios debería ser más accesible, clara y conectada." : "Business ownership should be more accessible, understandable, and connected."}</h1>
          <p>{es ? "Crestview existe para ayudar a más personas a encontrar, evaluar, adquirir y operar buenos negocios sin perderse en herramientas y procesos desconectados." : "Crestview exists to help more people find, evaluate, acquire, and operate good businesses without getting lost in disconnected tools and processes."}</p>
        </div>
        <div className="vision-hero__art" role="img" aria-label={es ? "Un camino conectado desde encontrar un negocio hasta operarlo" : "A connected path from finding a business to operating it"}>
          <div className="vision-hero__orbit" aria-hidden="true"><span /><span /><span /></div>
          <div className="vision-hero__journey" aria-hidden="true">
            {journey.map((item, index) => <span key={item}><b>0{index + 1}</b>{item}</span>)}
          </div>
        </div>
      </div>
    </section>
