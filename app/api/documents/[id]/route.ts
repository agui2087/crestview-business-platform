...
export async function GET(_: Request, context: Context) {
  ...
  const object = await getDocumentStorage().get(match.document.storageKey); if (!object) return Response.json({ error:"File not found." }, { status:404 });
...
export async function PATCH(request: Request, context: Context) {
  ...
  await getDb().update(documents).set({ originalName:nextName, category:validCategory(body.category ?? match.document.category), dealName:body.dealName === undefined ? match.document.dealName : safeName(body.dealName).slice(0,100) || null, fiscalYear:body.fiscalYear === undefined ? match.document.fiscalYear : body.fiscalYear.replace(/[^0-9]/g,"").slice(0,4) || null, updatedAt:new Date().toISOString() }).where(eq(documents.id, match.document.id));
...
export async function PUT(request: Request, context: Context) {
  ...
  await getDocumentStorage().put(key,file.stream(),{httpMetadata:{contentType:file.type}});
  await getDocumentStorage().delete(match.document.storageKey);
  await getDb().update(documents).set({ storageKey:key, originalName:name, contentType:file.type, sizeBytes:file.size, updatedAt:new Date().toISOString() }).where(eq(documents.id,match.document.id));
  await recordActivity(match.owner,match.document.id,"replaced",name); return Response.json({ok:true});
}

export async function DELETE(_: Request, context: Context) {
  const match = await owned(context); if (!match) return Response.json({ error:"Document not found." }, { status:404 });
  await getDocumentStorage().delete(match.document.storageKey); await getDb().delete(documents).where(eq(documents.id,match.document.id));
