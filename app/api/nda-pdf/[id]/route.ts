...
export async function GET(req:Request,{params}:{params:Promise<{id:string}>}) {
 ...
 const query=template?db.from('listing_nda_templates').select('storage_path, signing_layout').eq('listing_id',id).eq('broker_id',user.id):db.from('deal_ndas').select('storage_path, signing_layout').eq('id',id).or(`buyer_id.eq.${user.id},broker_id.eq.${user.id}`);
 ...
 if(!template&&row.signing_layout&&!matchesRecordedPdf(bytes,row.signing_layout.sha256))return new Response('The original PDF no longer matches this signing request.',{status:409,headers});
 return new Response(bytes,{headers:{...headers,'Content-Type':'application/pdf','Content-Disposition':'inline; filename="agreement.pdf"'}});
}
