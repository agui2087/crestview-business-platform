...
export function NdaFileInput({locale,required=false}:{locale:string;required?:boolean}) {
  ...
  return <div className="nda-file-picker">
    ...
    <small id={`${id}-help`}>{es?'Solo PDF, máximo 3.5 MB. Después de seleccionar el archivo, guarda el NDA.':'PDF only, maximum 3.5 MB. After selecting your file, save the NDA.'}</small>
  </div>;
}
