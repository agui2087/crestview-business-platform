...
export function NdaPdfWorkspace({mode,locale,source,recordId,version,layout,title}:{mode:'prepare'|'sign';locale:string;source:string;recordId:string;version:number;layout:NdaLayout|null;title:string}) {
 ...
 useEffect(()=>{
  ...
  void(async()=>{try{
   setBusy(true);setRendered(0);const p=await document.getPage(page);if(cancelled||!canvas.current)return;
  ...
  }catch{if(!cancelled){setError(t('This page could not be rendered. Do not sign until you can review it.','No se pudo mostrar esta página. No firmes hasta poder revisarla.'));setBusy(false);}}})();
 ...
 function nextField(){const next=fields.find(f=>!completed.includes(f.id));if(next){setPage(next.page);setSelected(next.id);}}
 async function submit(){
  setSaving(true);setError('');try{
   ...
   if(result.url)window.location.assign(result.url);else setError(result.error??t('Please try again.','Inténtalo de nuevo.'));
