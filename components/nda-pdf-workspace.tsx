...
import {parseNdaLayout,fieldRole,type NdaField,type NdaLayout,type SignerRole} from '@/lib/nda-fields';
...
export function NdaPdfWorkspace({mode,locale,source,recordId,version,layout,title,role='buyer',previous={},presets=[]}:{mode:'prepare'|'sign';locale:string;source:string;recordId:string;version:number;layout:NdaLayout|null;title:string;role?:SignerRole;previous?:Record<string,string>;presets?:{id:string;name:string;layout:NdaLayout}[]}) {
 ...
 const active=fields.find(f=>f.id===selected),label=(type:NdaField['type'])=>({signature:t('Signature','Firma'),initials:t('Initials','Iniciales'),date:t('Date (UTC)','Fecha (UTC)'),name:t('Full name','Nombre completo'),company:t('Company','Empresa'),title:t('Job title','Cargo'),text:t('Text','Texto'),checkbox:t('Checkbox','Casilla')})[type];
 ...
 function nextField(){const next=ownFields.find(f=>!completed.includes(f.id));if(next){setPage(next.page);setSelected(next.id);setFocusField(next.id);}}
 ...
 return <section className={styles.workspace} aria-label={t('Visual NDA workspace','Espacio visual del NDA')}>
  ...
  <div className={styles.columns}>
   <aside className={styles.controls}>
    {preparing?<><h2>{t('Place fields','Colocar campos')}</h2><p>{t('Add a field, then drag it into place. You can also set its position with the controls below. Assign fields to the buyer or broker. All fields are required.','Añade un campo y arrástralo a su posición, o usa los controles inferiores. Asigna campos al comprador o corredor. Todos son obligatorios.')}</p><label>{t('Signing order','Orden de firma')}<select aria-label={t('Signing order','Orden de firma')} value={order} onChange={e=>setOrder(e.target.value as NdaLayout['order'])}><option value="buyer_first">{t('Buyer first','Comprador primero')}</option><option value="broker_first">{t('Broker first','Corredor primero')}</option><option value="any">{t('Either order','Cualquier orden')}</option></select></label><div className={styles.add}>{(['signature','initials','date','name','company','title','text','checkbox'] as const).map(type=><button disabled={busy||saving||!document||fields.length>=50} type="button" key={type} onClick={()=>add(type)}>{t('Add','Añadir')} {label(type)}</button>)}</div>
    {active&&<fieldset disabled={saving}><legend>{label(active.type)} · {t('Page','Página')} {active.page}</legend><label>{t('Assigned signer','Firmante asignado')}<select aria-label={t('Assigned signer','Firmante asignado')} value={fieldRole(active)} onChange={e=>update(active.id,{role:e.target.value as SignerRole})}><option value="buyer">{t('Buyer','Comprador')}</option><option value="broker">{t('Broker','Corredor')}</option></select></label><label>{t('Field label','Etiqueta del campo')}<input maxLength={60} value={active.label??''} onChange={e=>update(active.id,{label:e.target.value})}/></label>{(['x','y','width','height'] as const).map(key=><label key={key}>{({x:t('Left (%)','Izquierda (%)'),y:t('Top (%)','Arriba (%)'),width:t('Width (%)','Ancho (%)'),height:t('Height (%)','Alto (%)')})[key]}<input type="number" min={key==='width'?8:key==='height'?2.5:0} max={key==='width'?80:key==='height'?20:100} step="0.5" value={Math.round(active[key]*1000)/10} onChange={e=>update(active.id,{[key]:Number(e.target.value)/100})}/></label>)}<button type="button" onClick={()=>{setFields(fs=>fs.filter(f=>f.id!==active.id));setSelected('');}}>{t('Remove field','Eliminar campo')}</button></fieldset>}
    ...
    <><h2>{t('Your signature','Tu firma')}</h2><label>{t('Full legal name','Nombre legal completo')}<input maxLength={100} value={name} disabled={saving} onChange={e=>{setName(e.target.value);setCompleted([]);setAccepted(false);}} autoComplete="name"/></label>{ownFields.some(f=>f.type==='initials')&&<label>{t('Your initials','Tus iniciales')}<input maxLength={12} value={initials} disabled={saving} onChange={e=>{setInitials(e.target.value);setCompleted([]);setAccepted(false);}}/></label>}<NdaSignatureInput es={es} value={appearance} disabled={saving} onChange={v=>{setAppearance(v);setCompleted([]);setAccepted(false);}}/>{ownFields.filter(f=>['company','title','text','checkbox'].includes(f.type)).map(f=><label key={f.id}>{f.label||label(f.type)}{f.type==='checkbox'?<input type="checkbox" checked={extra[f.id]==='Yes'} disabled={saving} onChange={e=>{setExtra({...extra,[f.id]:e.target.checked?'Yes':''});setCompleted(c=>c.filter(id=>id!==f.id));setAccepted(false);}}/>:<input maxLength={100} value={extra[f.id]??''} disabled={saving} onChange={e=>{setExtra({...extra,[f.id]:e.target.value});setCompleted(c=>c.filter(id=>id!==f.id));setAccepted(false);}}/>}</label>)}<p>{t('Click each marked field to apply your signature and required information. Nothing is signed until you confirm below.','Pulsa cada campo marcado para aplicar tu firma y los datos obligatorios. Nada queda firmado hasta confirmar abajo.')}</p><progress max={ownFields.length} value={completed.length} aria-label={t('Required fields completed','Campos obligatorios completados')}/><p role="status">{completed.length} / {ownFields.length} {t('fields completed','campos completados')}</p><button type="button" onClick={nextField} disabled={saving||completed.length===ownFields.length}>{t('Go to next required field','Ir al siguiente campo obligatorio')}</button><label className={styles.consent}><input type="checkbox" checked={accepted} disabled={saving} onChange={e=>setAccepted(e.target.checked)}/>{t('I have reviewed the complete agreement and agree to sign it electronically using the name and initials shown.','He revisado el acuerdo completo y acepto firmarlo electrónicamente con el nombre y las iniciales indicados.')}</label><button className={styles.primary} type="button" disabled={!pageReady||saving||!accepted||completed.length!==ownFields.length||name.trim().length<2} onClick={()=>void submit()}>{saving?t('Creating signed PDF…','Creando PDF firmado…'):t('Confirm and sign NDA','Confirmar y firmar NDA')}</button><p>{t('The date is recorded in UTC. A completed PDF is available after every assigned party signs.','La fecha se registra en UTC. El PDF completado estará disponible cuando firmen todas las partes.')}</p></>}
   ...
   <div className={styles.document} tabIndex={0} role="region" aria-label={t('Scrollable PDF page','Página PDF desplazable')}>{busy&&<p role="status">{t('Loading PDF page…','Cargando página del PDF…')}</p>}<div className={styles.sheet} style={{width:`${zoom*100}%`}} ref={sheet} onPointerMove={e=>{
    ...
    {rendered===page&&fields.filter(f=>f.page===page).map(f=><button type="button" key={f.id} data-nda-field={f.id} aria-label={`${preparing?t('Position','Posicionar'):t('Apply','Aplicar')} ${label(f.type)} ${fields.indexOf(f)+1}`} aria-pressed={selected===f.id||completed.includes(f.id)} className={`${styles.field} ${completed.includes(f.id)?styles.completed:''}`} style={{left:`${f.x*100}%`,top:`${f.y*100}%`,width:`${f.width*100}%`,height:`${f.height*100}%`,fontStyle:f.type==='signature'&&completed.includes(f.id)?'italic':undefined}} disabled={saving||busy||(!preparing&&fieldRole(f)!==role)} onPointerDown={e=>{if(!preparing)return;setSelected(f.id);e.currentTarget.setPointerCapture(e.pointerId);drag.current={id:f.id,startX:e.clientX,startY:e.clientY,x:f.x,y:f.y};}} onClick={()=>{setSelected(f.id);if(!preparing)apply(f);}} onKeyDown={e=>{if(!preparing)return;const delta=e.shiftKey?.02:.005;if(['ArrowLeft','ArrowRight','ArrowUp','ArrowDown'].includes(e.key)){e.preventDefault();update(f.id,{x:Math.max(0,Math.min(1-f.width,f.x+(e.key==='ArrowRight'?delta:e.key==='ArrowLeft'?-delta:0))),y:Math.max(0,Math.min(1-f.height,f.y+(e.key==='ArrowDown'?delta:e.key==='ArrowUp'?-delta:0)))});}}}>{completed.includes(f.id)||previous[f.id]?f.type==='signature'&&completed.includes(f.id)&&appearance.mode==='drawn'?<svg viewBox="0 0 600 180" width="100%" height="100%" aria-label={t('Drawn signature','Firma dibujada')}>{appearance.strokes.map((s,i)=><polyline key={i} points={s.map(p=>`${p[0]*600},${p[1]*180}`).join(' ')} fill="none" stroke="currentColor" strokeWidth="3"/>)}</svg>:f.type==='signature'&&completed.includes(f.id)&&appearance.mode==='uploaded'?<span role="img" aria-label={t('Uploaded signature','Firma subida')} style={{display:'block',width:'100%',height:'100%',backgroundImage:`url(${appearance.image})`,backgroundSize:'contain',backgroundPosition:'center',backgroundRepeat:'no-repeat'}}/>:fieldText(f):`${f.label||label(f.type)}${fieldRole(f)==='broker'?t(' · Broker',' · Corredor'):''}`}</button>)}
