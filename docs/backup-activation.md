...
The monitor also checks every six hours (subject to GitHub scheduling delays). It flags missing history, a passing drill older than eight days, and missing, empty or expired encrypted artifact metadata. Eight days allows one day of grace for the current weekly backup schedule; it does not meet or certify the separate 24-hour recovery-point objective. A passing drill cannot close the incident unless its artifact is still available. Metadata availability is not a new decryption or restore test.

The monitor uses reviewed main-branch code, never the triggering branch's code, artifacts, or logs. It receives no backup credentials and reads artifact metadata only. Public incident text contains only a fixed description, outcome and run link—not customer data, connection strings or raw errors. GitHub notification delivery depends on the repository's configured subscriptions; this is not a verified email, SMS or on-call service. If GitHub schedules themselves stop running, this monitor cannot independently detect its own outage. Continue checking the age and usability of the latest saved backup independently.
