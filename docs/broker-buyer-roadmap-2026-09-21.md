...
|10|Respectful follow-up|Broker declines require a shared explanation; stage, explanation and history save atomically (PR104 live). Buyer-controlled per-inquiry pause/resume now has participant-only visibility, immutable audit history and broker notice. It mutes routine message notices without hiding messages or changing agreements, deadlines or document access. 292 automated tests, build/lint and connected EN/ES desktop/mobile pause/resume rehearsal passed. Pause deployment tracked separately; future-contact opt-in remains.|
...
|15|Due-diligence questions|Private full/unresolved CSV exports added with owner filtering, pagination, explicit failures, spreadsheet-formula protection and private-note warnings. Received records remain unresolved until the buyer marks verification; N/A and verified records excluded from unresolved exports. Connected buyer/broker/anonymous isolation passed. Advisor invitations and structured answer linking remain.|
...
|20|Notifications|Found device-local alert controls that did not govern delivery. Replaced with private account-saved preferences enforced on future message/document/status notices. Essential signing/security/access notices remain. Database isolation and connected mute-with-message-preserved test passed. New-match/price/task alerts are explicitly not enabled. Email provider approval, sender verification and live delivery remain outstanding.|
