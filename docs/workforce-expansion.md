...
Through PR49, production includes per-business setup and employee placement, effective-dated planned schedules, reviewed leave policies/manual ledger with request-linked debits and atomic carryover, reviewed payroll-analysis imports with audited voids and complete period/currency aggregates, and explicit revision-bound training-evidence links. These do not imply automated legal entitlements, payroll processing, email delivery or complete multi-role browser acceptance.
...
The integration layer, `lib/workforce-form-integration.test.ts`, executes the actual operations, leave, payroll and training-evidence Server Action handlers with FormData against isolated PostgreSQL migrations. It covers owner, HR, assigned/unassigned manager, employee and outsider identities: invitation acceptance, leave decisions, training completion/independent verification, carryover confirmation/replay, payroll review/validation/duplicate prevention/void-and-replace, evidence confirmation/blocked-file and private-ownership restrictions/stale task versions/revocation, revoked access and signed-out redirects. Next navigation/session/Supabase transport are test substitutes; database authorization and application handler logic are real. Vault security metadata is synthetic; the scanner/storage service is not exercised by this test. These are **not** hosted browser E2E tests. Local environment inspection found no Supabase credentials configured; no production credentials or real records were used.
