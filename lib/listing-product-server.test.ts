...
test('listing checkout server validates actual payment and preserves retry identity',async t=>{
  ...
  const order={id:orderId,listing_id:listing,user_id:user,product_code:'single_listing',price_id:'price_fixture',customer_id:'cus_fixture',status:'pending',checkout_session_id:null as string|null,checkout_expires_at:'2030-01-01T00:35:00Z'};
  ...
  const stripe={prices:{retrieve:async()=>({active:true,currency:'usd',unit_amount:badPrice?1:3000,recurring:null})},
    checkout:{sessions:{retrieve:async()=>session(),listLineItems:async()=>({has_more:false,data:[{price:{id:'price_fixture'},quantity}]}),create:async(params:Record<string,unknown>,options:Record<string,unknown>)=>{calls.push({name:'stripe.create',params:{...params,...options}});return session();}}},
