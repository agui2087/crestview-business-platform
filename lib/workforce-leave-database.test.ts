...
test('leave postings are scoped, append-only, idempotent and capped',async()=>{
  ...
  try {
    ...
    for(const name of ['0008_workforce_and_organizations','0029_workforce_record_integrity','0030_workforce_operations','0035_workforce_leave_ledger'])await db.exec(await readFile(new URL(`../supabase/migrations/${name}.sql`,import.meta.url),'utf8'));
    await as(owner);
    ...
    await as(owner);await assert.rejects(post('adjustment',1,'audit-failure'));
    assert.equal((await db.query('select * from workforce_leave_ledger')).rows.length,603);
