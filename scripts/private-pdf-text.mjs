...
try{
  const task=getDocument({data:new Uint8Array(Buffer.concat(chunks)),verbosity:0,isEvalSupported:false,disableFontFace:true,useSystemFonts:false});
  ...
  for(let n=1;n<=pdf.numPages;n++){
    ...
    if(text.length>12000||Buffer.byteLength(text,'utf8')>6000)throw new Error('page_too_large');
