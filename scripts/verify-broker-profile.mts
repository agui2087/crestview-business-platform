...
try{
 ...
 for(const locale of ['en','es'])for(const width of [1280,390]){
  await visitor.setViewportSize({width,height:900});await visitor.goto(`${base}/${locale}/brokers/${broker.id}`);expect(await visitor.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1)).toBe(true);
