...
begin
  ...
  if auth.uid() is null or inquiry_row.id is null or inquiry_row.broker_id is distinct from auth.uid() then
...
revoke all on function public.get_broker_buyer_summary(uuid) from public, anon;
